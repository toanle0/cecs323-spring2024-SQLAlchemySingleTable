...
def add_department(session: Session):
    ...
    existing_department = session.query(Department).filter(
        ...
        (Department.chair_name == chair_name) |
        ((Department.building == building) & (Department.office == office)) |
        (Department.description == description)
    ).first()

    if existing_department:
        print("Department violates uniqueness constraints. Try again.")
    else:
...
def select_department(session: Session) -> Department:
    ...
    while not found:
        ...
        if department:
            found = True
            print(f"Department selected: {department.name}")
        else:
...
def delete_department(session: Session):
    """
    Prompt the user for a department by a unique attribute (e.g., name) and delete that department.
    :param session: The connection to the database.
    :return: None
    """
    ...
    if department:
        ...
        session.commit()
        print(f"Department '{dept_name}' deleted successfully.")
    else:
